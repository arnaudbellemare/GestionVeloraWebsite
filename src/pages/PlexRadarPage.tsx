...
const copy = {
  fr: {
    ...
    results: "résultats · triés par score publié",
  ...
  en: {
    ...
    results: "results · sorted by published score", cap: "Cap rate", cash: "Cash flow / mo", score: "Score",
      ...
      <section className="radar-workspace">
        <div className="radar-list-side">
          <div className="radar-filters">
            ...
            <select aria-label={t.all} value={region} onChange={(event) => setRegion(event.target.value)}>{regions.map((item) => <option key={item}>{item}</option>)}</select>
            <label><input type="checkbox" checked={positive} onChange={(event) => setPositive(event.target.checked)} /> {t.positive}</label>
          </div>
          <p className="radar-count">{filtered.length} {t.results}</p>
          <div className="radar-list">
            {filtered.map((deal, index) => <button key={deal.listing.listing_id} className={selected?.listing.listing_id === deal.listing.listing_id ? "is-selected" : ""} onClick={() => setSelectedId(deal.listing.listing_id)}>
              <span>{String(index + 1).padStart(2, "0")}</span><span><strong>{deal.listing.address}</strong><small>{deal.listing.property_type} · {deal.listing.city} · {deal.listing.units} portes</small></span><span><strong>{money.format(deal.listing.price)}</strong><small>{deal.live ? `${pct.format(deal.live.capRate)} ${t.cap}` : "—"}</small></span><span><strong>{deal.live ? money.format(deal.live.monthlyCashFlow) : "—"}</strong><small>{t.cash}</small></span><em>{deal.live?.score ?? "—"}/12</em>
            </button>)}
          </div>
        </div>

        {selected?.live && <aside className="radar-analysis">
          <div className="radar-analysis-head"><div><p>{selectedExcluded.length ? t.scenario : t.published}</p><h2>{selected.listing.address}</h2><small>{selected.listing.city} · {selected.listing.units} portes · {selected.listing.year_built ?? "—"}</small></div><strong>{selected.live.score}<small>/12</small></strong></div>
