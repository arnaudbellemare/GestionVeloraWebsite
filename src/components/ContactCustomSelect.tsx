...
export function ContactCustomSelect({
  ...
  return (
    ...
    >
      ...
      >
        <span id={`${id}-value`} className={showPlaceholder ? "text-nd-secondary" : ""}>
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            ...
            className="absolute left-0 right-0 top-full z-[2] isolate w-full overflow-hidden rounded-b-md border border-t-0 border-nd-border-visible bg-nd-canvas"
          >
            <ul
              id={listId}
              role="listbox"
                  ...
                  <li key={opt.value === "" ? "__empty__" : opt.value} role="presentation" className="px-1">
                    <button
                      ...
                      className={`flex w-full items-center gap-2 rounded-md px-3 py-2.5 text-left text-sm transition-colors ${
                        active
                          ? "bg-nd-surface text-nd-primary"
                          : "text-nd-primary hover:bg-nd-surface"
